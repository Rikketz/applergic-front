...
export default function ResultPage({ detectedCode, userId }) {
  ...
  useEffect(() => {
    if (detectedQRCode && productData.length > 0 && !foundProduct) {
      ...
      if (matchingProduct) {
        ...
        const listaAlergias = alergiaList || [];
        const productAllergens = matchingProduct.alergenosPresentes || [];
  ...
  }, [detectedQRCode, productData, alergiaList, setCodigoParaPasar]);
  const goBack = () => {
    window.history.back();
  };
        ...
        <React.Fragment>
          ...
          <div className="result-p">
            {foundProduct ? (
              ...
              <p className="result-n">
                Lo sentimos, no hay datos suficientes para poder valorar este producto
              </p>
            )}
            <p className="p-n">
              Este producto no es apto para ti, contiene {foundProduct?.alergenosPresentes.join(", ")}
            </p>
          </div>
          ...
          <div>
            <div>
              ...
              <p className="ap">
                Alergenos presentes: {foundProduct?.alergenosPresentes?.join(", ")}
              </p>
            </div>
          </div>
